 ...
 * @param {boolean} injectAPIs          //new flag to inject APIs
 * @param {Object.<string, string>} collectorFlags
 */
async function crawlAndSaveData(urlString, dataCollectors, log, filterOutFirstParty, dataCallback, emulateMobile, proxyHost, antiBotDetection, executablePath, maxLoadTimeMs, extraExecutionTimeMs, collectorFlags, injectAPIs) {
    ...
    const data = await crawl(url, {
        ...
        collectorFlags,
        injectAPIs          //new flag to inject APIs
    });
...
module.exports = async options => {
    ...
    async.eachOfLimit(options.urls, numberOfCrawlers, (urlItem, idx, callback) => {
        ...
        const task = crawlAndSaveData.bind(null, urlString, dataCollectors, log, options.filterOutFirstParty, options.dataCallback, options.emulateMobile, options.proxyHost, (options.antiBotDetection !== false), executablePath, options.maxLoadTimeMs, options.extraExecutionTimeMs, options.collectorFlags, options.injectAPIs);
