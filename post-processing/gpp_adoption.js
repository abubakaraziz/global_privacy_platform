...
function processFile(filePath) {
    try {
        ...
        // Check if gppObjects exists and contains data
        if (data && data.gpp && Array.isArray(data.gpp.gppObjects) && data.gpp.gppObjects.length > 0) {
            const gppObject = data.gpp.gppObjects[0];
            gppApiEntries.push({
                url: initialUrl,
                gppObject
...
const main = () => {
    const outputDir = path.dirname(options.o);
    if (!fs.existsSync(outputDir)) {
        fs.mkdirSync(outputDir, {recursive: true});
    }

    fs.readdirSync(options.i).forEach(file => {
        const filePath = path.join(options.i, file);
        processFile(filePath);
    });
    ...
    console.log('Processing completed. Results saved in ' + options.o);
};
